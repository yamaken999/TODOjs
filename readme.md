...
1.  **ブラウザで開く**:
    VS Codeの左側のファイルエクスプローラーで、**`index.html`ファイルをGoogle Chromeのウィンドウにドラッグ＆ドロップ**します。
...
作成したアプリをインターネット上に公開し、世界中のどこからでもアクセスできるようにします。スマホからも確認できるようになります！
...
現在のアプリは、ブラウザを閉じてもデータが消えないように`localStorage`を使っていますが、これはアクセスしているデバイス（PCやスマホ）ごとにデータが保存されます。そのため、PCで追加したTODOがスマホに自動で表示される、といった「同期」はできません。
...
興味があれば、AIに「Firebaseを使ったTODOアプリのコードを書いて」と聞いてみたり、オンラインデータベースについて調べてみてくださいね！
